...
关键词查询同时用 `paradedb.const_score(0, ...)` 表达这两个过滤条件。仅新增索引而保留原 SQL，会让过滤条件参与 BM25 评分；零分过滤保留内容相关性排序，并通过排除 `false` 保留历史 `NULL` 数据。应用查询与这条 migration 需要一起部署；回滚数据库时也应恢复对应的旧应用版本。
